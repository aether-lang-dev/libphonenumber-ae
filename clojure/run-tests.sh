...
if [ -n "$CLJ_CP" ]; then
    exec "$RUN_JAVA" --enable-native-access=ALL-UNNAMED \
        -cp "${CLJ_CP}${PN_JAVA_CLASSES}:$SRC" \
        clojure.main -m org.libphonenumber.ae.conformance-test
fi
...
#    /usr/bin/clojure that is a DIFFERENT tool — it does not understand -Scp and
#    dies with "-Scp (No such file or directory)", which reads as a test failure
#    rather than a missing toolchain. Verify with -Sdescribe (cheap, read-only)
#    before trusting it.
cli=""
...
if [ -n "$cli" ]; then
    ...
    JAVA_OPTS="--enable-native-access=ALL-UNNAMED" \
        exec "$cli" -Scp "$PN_JAVA_CLASSES:$SRC" \
        -M -m org.libphonenumber.ae.conformance-test
fi
