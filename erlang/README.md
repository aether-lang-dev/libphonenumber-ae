...
A thin Erlang binding over the shared, pure-Aether libphonenumber engine. All the
...
`ERL_LIBS` — the engine `.so` is vendored under `priv/`, so nothing else is needed
at runtime:
...
`dlopen`s the engine at load time in this order: `$LIBPHONENUMBER_AE_LIB`, then
