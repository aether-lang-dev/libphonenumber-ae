#!/usr/bin/env sh
# get-core.sh — download the prebuilt libphonenumber_ae core for THIS machine
# from a GitHub Release, verify its checksum, and print its path.
#
# This is the "no clone, no aeb, no Aether" path: a binding needs the core
...
#
# Usage:
#   ./get-core.sh                       # latest release, into ./ (prints the path)
#   ./get-core.sh v1.2.3                # a specific tag
#   ./get-core.sh v1.2.3 /opt/lib       # into a directory of your choice
#   LIBPHONENUMBER_AE_TAG=v1.2.3 DEST=~/lib ./get-core.sh
#
...
case "$(uname -s 2>/dev/null)" in
  ...
  *) die "unrecognized OS $(uname -s) — download a core asset manually from https://github.com/$REPO/releases" ;;
esac
case "$(uname -m 2>/dev/null)" in
  x86_64|amd64)   _arch=x86_64 ;;
  arm64|aarch64)  _arch=arm64 ;;
  *) die "unrecognized CPU $(uname -m) — download a core asset manually from https://github.com/$REPO/releases" ;;
esac
...
printf 'get-core: fetching %s (%s-%s, tag %s)\n' "$ASSET" "$_os" "$_arch" "$TAG" >&2
curl -fsSL "$BASE/$ASSET"        -o "$OUT"      || die "download failed: $BASE/$ASSET (no asset for this platform/tag?)"
curl -fsSL "$BASE/$ASSET.sha256" -o "$OUT.sha256" 2>/dev/null || printf 'get-core: no .sha256 sidecar to verify against\n' >&2
