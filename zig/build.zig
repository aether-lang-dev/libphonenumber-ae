...
//!
//! This binding LINKS the core (like Go's cgo binding, unlike the
//! dlopen-based Python/Rust ones), so `libphonenumber_ae.so` must exist at
//! *build* time. Everything below is about telling the linker where it is at
//! build time AND baking an rpath so the produced binary finds it at run time
//! without an `LD_LIBRARY_PATH` incantation.
//!
//! Core search order (first hit wins):
//!   1. `-Dengine=/abs/path/to/libphonenumber_ae.so` — what `.tests.ae` uses,
...
//!
//!     zig build test                       # in-tree, core already built
...
pub fn build(b: *std.Build) void {
    ...
    const engine_opt = b.option([]const u8, "engine", "Path to libphonenumber_ae.so (or the directory holding it)");
    const dirs = engineSearchPath(b, engine_opt);

    // The module, so a downstream package can `@import("phonenumber_ae")`.
    // Note it is the *consumer's* build that must link the core — a module
    // carries source, not link flags — which the README spells out.
    _ = b.addModule("phonenumber_ae", .{
    ...
    // the core .so — Zig's build graph cannot see.
    run_tests.has_side_effects = true;

    const test_step = b.step("test", "Run the 34-check v2 conformance suite");
    ...
    example_mod.addImport("phonenumber_ae", b.createModule(.{
        .root_source_file = b.path("src/root.zig"),
        .target = target,
        .optimize = optimize,
    }));
    const example = b.addExecutable(.{
        .name = "phonenumber-ae-example",
...
/// Wire one compile step up to the core: libc (the ABI is C), the search
...
fn linkEngine(b: *std.Build, step: *std.Build.Step.Compile, dirs: []const []const u8) void {
    ...
    // `extern "c"` declarations need libc.
    mod.link_libc = true;
    for (dirs) |d| {
    ...
    mod.linkSystemLibrary("phonenumber_ae", .{});
    _ = b;
}

/// Resolve the core search path, most-specific first. Returns directories.
fn engineSearchPath(b: *std.Build, engine_opt: ?[]const u8) []const []const u8 {
    ...
    if (b.graph.environ_map.get("LIBPHONENUMBER_AE_LIB")) |e| {
