#!/usr/bin/env sh
# Load the PhonenumberAe package into a throwaway Pharo image and run SUnit
# headless.
#
...
LIBPHONENUMBER_AE_LIB=${LIBPHONENUMBER_AE_LIB:-"$root/core/native/libphonenumber_ae.so"}
...
cp "$IMAGE" "$work/PhonenumberAe.image"
base=$(dirname "$IMAGE")/$(basename "$IMAGE" .image)
[ -f "$base.changes" ] && cp "$base.changes" "$work/PhonenumberAe.changes"
...
"$PHARO_BIN" --headless "$work/PhonenumberAe.image" st --quit "$work/load.st" \
    || { echo "pharo: package load failed"; exit 1; }

# `test` exits non-zero if any test fails, which is what we key off.
"$PHARO_BIN" --headless "$work/PhonenumberAe.image" test --junit-xml-output PhonenumberAe
rc=$?
